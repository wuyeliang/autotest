...
class factory_AudioQuality(test.test):
    ...
    def listen_forever(self, sock):
        ...
        while True:
            ...
            counter += 1
            factory.console.info('Handle connection, counter %d' % counter)
            if fd in _rl:
                self.ui.call_js_function('setMessage',
                        'Connected the %d time' % counter)
                conn, addr = sock.accept()
                self.handle_connection(conn)
                factory.console.info('Done with one connection.')
    ...
    def start_server(self):
        ...
        self.ui.call_js_function('setMessage',
                'Ready for connection | 準備完成,等待联结')
